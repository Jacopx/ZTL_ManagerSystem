...
//			String requestBody = "{\"statements\" : [ {\"statement\" : \"MATCH (n) RETURN count(*)\"} ]}";
			...
			Results response =client.target(base_url).path("transaction/commit").request(MediaType.APPLICATION_JSON).post(Entity.entity(statements,MediaType.APPLICATION_JSON),Results.class);
